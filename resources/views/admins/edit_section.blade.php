    ...
    <div class="max-w-7xl m-4 mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
            <div class="mx-auto bg-gray-100">
                <h2 class="text-2xl font-bold card bg-green-600 p-4 text-gray-100 rounded-t-lg mx-auto">Phần mới</h2>
                <div class="mt-2 max-w-auto mx-auto card p-4 bg-white rounded-b-lg shadow-md">
                    <div class="grid grid-cols-1 gap-6">
                        <form action="{{route('updateSection', $section->id)}}" method="post">
                            @csrf
                            <label class="block">
                                <span class="text-gray-700">Tên Phần</span>
                                @error('name')
                            ...
                            <label class="block">
                                <span class="text-gray-700">Mô tả phần</span>
                                @error('description')
                                <span class="text-red-700 text-xs content-end float-right">{{$message}}</span>
                                @enderror
                            ...
                            <label class="block">
                                <span class="text-gray-700">Phần này có hoạt động không?</span>
                                @error('is_active')
                                <span class="text-red-700 text-xs content-end float-right">{{$message}}</span>
                                @enderror
                            ...
                            <label class="block">
                                <span class="text-gray-700">Chi tiết Phần</span>
                                @error('details')
                                <span class="text-red-700 text-xs content-end float-right">{{$message}}</span>
                                @enderror
